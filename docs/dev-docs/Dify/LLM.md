...
![LLM Node Overview](https://assets-docs.dify.ai/dify-enterprise-mintlify/en/guides/workflow/node/85730fbfa1d441d12d969b89adf2670e.png)

LLM Node Overview

LLM node configuration interface

Configure at least one model provider in **System Settings → Model Providers** before using LLM nodes. See [how to install model plugins](https://docs.dify.ai/en/use-dify/workspace/plugins) for setup instructions.
...
![LLM Node Configuration](https://assets-docs.dify.ai/dify-enterprise-mintlify/en/guides/workflow/node/43f81418ea70d4d79e3705505e777b1b.png)
...
Using context variables for RAG applications
...
![](https://mintcdn.com/dify-6c0370d8/MiGqQWUuc2Ca3g05/images/llm-memory.png?w=280&fit=max&auto=format&n=MiGqQWUuc2Ca3g05&q=85&s=00e6be5285b2a6599582525daa0ca63e)

System: You are a technical documentation expert. User: {{user\_input}}
...
![File Processing](https://assets-docs.dify.ai/2024/11/05b3d4a78038bc7afbb157078e3b2b26.png)

File Processing

File processing with multimodal LLMs

For conversation history in completion models, insert conversation variables to maintain multi-turn context:

![Conversation History](https://assets-docs.dify.ai/dify-enterprise-mintlify/en/guides/workflow/node/b8642f8c6e3f562fceeefae83628fd68.png)

Conversation History

Using conversation history variables
...
LLM nodes support streaming output by default. Each text chunk is yielded as a `RunStreamChunkEvent`, enabling real-time response display. File outputs (images, documents) are processed and saved automatically during streaming.Configure retry behavior for failed LLM calls. Set maximum retry attempts, intervals between retries, and backoff multipliers. Define fallback strategies like default values, error routing, or alternative models when retries aren’t sufficient.

[Previous](https://docs.dify.ai/en/use-dify/nodes/trigger/webhook-trigger)[Knowledge Retrieval](https://docs.dify.ai/en/use-dify/nodes/knowledge-retrieval)
